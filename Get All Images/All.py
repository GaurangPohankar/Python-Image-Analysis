...
def resize(image):
     img = Image.open(image) # image extension *.png,*.jpg
     new_width  = 200
     new_height = 300
...
def noisecancel(image):
     ...
     noise = cv2.imwrite(image, image_channels)
...
def sharp(image):
     ...
     imgIn = cv2.imread(image, cv2.IMREAD_GRAYSCALE)
